...
export default class {
  ...
  update(opt) {
    ...
    this.setOption(this.getArgs(opt))
  }
  // 获取入参配置
  getArgs(opt) {
    this.opt = opt
  ...
  noData() {
    ...
    img.src = noDataImg
    img.onload = () => {
      const imgW = this.small && img.width * 0.75 || img.width
      const imgH = this.small && img.height * 0.75 || img.height
      cxt.drawImage(
        img,
        (box.width - imgW) / 2,
        (box.height - imgH) / 2,
  ...
  setOption(args) {
    ...
    if (!args.onClick && !args.dbClick) {
      options.series && Array.isArray(options.series) &&
      options.series.forEach(s => s.cursor = 'default')
    }
    // 检查数据是否为有效数据
    if (!this.checkSeries(options)) return
    this.charts.setOption(options)
    this.addDomListenerRisize(this.chartDom, () => {
      this.resize()
      if (typeof this.opt === 'function') {
        this.charts.setOption(this.options(this.opt()))
      }
    })
  }
  options({ data = {}, options, deep }) {
    ...
    }, {...options}) // 将data 中的 series 属性合并到 options 中
    let deaultData = getDefaultOptions({...data}, {...options}, () => this.getOptions())
    if (deep) {
      if (deep.length) { // 指定 属性 深拷贝
        let copyData = {}
  ...
  checkSeries ({series, dataset, noCheckSeries}) {
    ...
    if (dataset) {
      ...
      // 使用series 时 判断数据是否有效
      if (series.some(s => {
        return s && Array.isArray(s.data) && s.data.some(d => {
  ...
  addDomListenerRisize (dom, callBack) {
    ...
    this.resizeTimers = setInterval(() => {
      if (
        (this.getDomSize(dom).width !== width ||
        this.getDomSize(dom).height !== height) &&
