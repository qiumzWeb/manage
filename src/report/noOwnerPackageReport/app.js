...
export default function App(props) {
  ...
  const beforeSearch = (req, action) => {
    ...
    // 包裹号，统计时间，三选一
    if (
      isEmpty(data.deliveryCodes) &&
      isEmptyTime([data.onShelveStartTime, data.onShelveEndTime]) &&
      isEmptyTime([data.offShelveStartTime, data.offShelveEndTime])
    ) return "上架时间或者下架时间不能为空"
  }
  ...
  return <div className='class_no_owner_package_report'>
    <Page
      ref={pageRef}
      // 自定义查询 自定表头 code
      code="no_owner_package_report"
    // 查询配置
      searchOptions={{
        url: searchApiUrl,
        method: 'post',
