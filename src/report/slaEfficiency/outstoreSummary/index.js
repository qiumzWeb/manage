  ...
  {/* 查询列表 */}
    <QueryList
      ...
      tableOptions={{
        ...
        method: 'post',
        attrs: {
          defineFieldCode: 'SLAEfficiencyOutStoreSummary'
        }
      }}
    >
      <div slot="tools">
        <ExportFile
          ...
          // btnProps={{mr: 0}}
          commandKey='OUT_STOCK_SLA_STATISTIC'
        ></ExportFile>
      </div>
      {Object.entries(goDetailsKeys).map(([key, type]) => {
        return <div slot="tableCell" prop={key} key={key}>
          {(val, index, record) => {
            ...
            return <Button text type="link" onClick={() => goDetail(record, type.value, 2)}>{val}</Button>
          }}
        </div>
      })}
    </QueryList>
