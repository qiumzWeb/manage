...
WarehouseConfigStep.title = '异常预警配置'
...
export default function WarehouseConfigStep() {
  ...
  const saveDraftKey = 'step170DraftData_' + baseData.openWarehouseCode;
  // 初始化默认值
  ...
  function isModify() {
    ...
    return baseData.currentSaveNode > 170
  }
  // 初始化数据
  useEffect(initStep, []);
    ...
    try {
      ...
      if (result) {
        const params = {
          ...
          timeScoped: [moment().subtract(1, 'years'), moment().add(10, 'years')]
        }
        const submitUrl = activeData.isAdd ? '/sys/storageWarning/config/add' : '/sys/storageWarning/config/update'
        const time = getRangTime(params, {time: 'timeScoped', start: 'timeScopeStart', end: 'timeScopeEnd'})
  ...
  return <div>
    ...
    <BottomTool
      leftTool={<>
        <Button size="large" s call_jump="160">上一步</Button>
      </>}
      rightTool={
        <>
          {
        ...
        { getStepBaseData().warehouseStatus < 20 && <Button size="large" p onClick={async () => {
          await saveStepNode(180, true).then(_=> {
            setStepBaseData({
              warehouseStatus: 20
            })
            Dialog.confirm({
