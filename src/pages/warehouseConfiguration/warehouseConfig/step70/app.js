...
import { signModel, packageSignModel, getWarehouseSignConfigList, rejectModel } from './config'
...
WarehouseConfigStep.title = '签收配置'
...
export default function WarehouseConfigStep() {
  ...
  const saveDraftKey = 'step70DraftData_' + baseData.openWarehouseCode;
  // 初始化默认值
  let defaultData = {uuid: getUuid()}
  InitDefaultData([signModel, packageSignModel], defaultData)
  Object.assign(defaultData, {
    signType: baseData.warehouseType == 1 ? '0' : '1',
    isBigbagSignWeighing: '0',
    isSmallbagSignAudit: '0',
    carTitleStatus: '0',
    isSmallRejectPhoto: '0',
  ...
  function isModify() {
    ...
    return baseData.currentSaveNode > 70
  }
  // 初始化数据
  useEffect(initStep, []);
  ...
  async function getInitData() {
    let res = await getWarehouseSignConfigList({
      warehouseId: baseData.warehouseId,
    })
    if (isEmpty(res)) {
      res = defaultData
  ...
  return <div>
    <FormGroup
      ...
      data={data}
      group={{
        sign: { title: '基础配置', model: signModel },
        bigSign: { title: '校验规则配置', model: packageSignModel },
        other: { title: '拒收配置', model: rejectModel}
      }}
    ></FormGroup>
    <BottomTool
      leftTool={<>
        <Button size="large" s call_jump="60">上一步</Button>
      </>}
      rightTool={
        isModify() ? 
          <>
            <Button size="large" p onClick={onOk} mr='10'>保存</Button>
            <Button size="large" s mr='10' call_jump="80">下一步</Button>
          </>
        : <>
          <Button size="large" s onClick={saveDraft} mr='10'>保存为草稿</Button>
          <Button size="large" p mr='10' onClick={onOk} call_jump="80">保存并下一步</Button>
        </>
      }
    ></BottomTool>
  </div>
