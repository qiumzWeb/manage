...
import { qSearch, tColumns } from './config/useWorkersConfig'
import { useWorkSearchApi, getUseWorkExport, userWorkSearchSumApi} from './api'
export default class Index extends React.Component {
  ...
  exportData = () => {
    try {
      ...
      getUseWorkExport(data.data).finally(() => {
        this.setState({
          exprotLoading: false
        })
      })
    ...
    return (
      <div className="pcsbox">
        ...
        <QueryList
          ...
          tableOptions={{
            url: useWorkSearchApi,
            sumUrl: userWorkSearchSumApi,
            method: 'post'
          }}
        >
          <span slot="tools">
            <Button type="secondary" mr="10" onClick={() => this.exportData()}>{this.state.exprotLoading && <Icon type="loading"/>} 导出</Button>
          </span>
        </QueryList>
      </div>
    )
